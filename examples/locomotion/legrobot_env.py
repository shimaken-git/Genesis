...
class LegRobotEnv:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, device="cuda"):
        ...
        self.scene = gs.Scene(
            ...
            # vis_options=gs.options.VisOptions(n_rendered_envs=1),     #old genesis
        ...
        link_foot = self.robot.get_link("foot")
        self.foot_idx = link_foot.idx -1
        self.foot_contact_airtime = torch.zeros((self.num_envs,), device = self.device, dtype=gs.tc_float)
    ...
    def step(self, actions):
        ...
        self.reset_buf |= torch.abs(self.base_euler[:, 1]) > self.env_cfg["termination_if_pitch_greater_than"] + 32.6
        ...
        self.obs_buf = torch.cat(
            [
                ...
                (self.dof_pos - self.default_dof_pos) * self.obs_scales["dof_pos"],  # 3
                self.dof_vel * self.obs_scales["dof_vel"],  # 3
                self.actions,  # 3
            ],
            axis=-1,
        )
    ...
    def _reward_feet_air_time(self):
        foot_contact_force = self.robot.get_links_net_contact_force()[:, self.foot_idx, 2]
        foot_contact_bool = (foot_contact_force == 0).to(torch.float32)
        # print("foot_contact_force", foot_contact_force)
        # print("foot_contact_bool", foot_contact_bool)
        self.foot_contact_airtime += foot_contact_bool
        self.foot_contact_airtime *= foot_contact_bool
        # print("foot_contact_airtime", torch.square(self.foot_contact_airtime))
        return torch.square(self.foot_contact_airtime)
